...
def boot_state_handler(event, context):
    """启动状态处理函数"""
    if event == "enter":
        info("状态: {} ,进入启动状态, 系统开始初始化", event, module="FSM")
        ...
        if time.ticks_diff(time.ticks_ms(), context.get("boot_start_time", 0)) >= 1000:
            info("状态: {} ,启动阶段完成, 发布boot_complete事件", event, module="FSM")
...
def init_state_handler(event, context):
    """初始化状态处理函数"""
    if event == "enter":
        info("状态: {} ,进入模块初始化...", event, module="FSM")
        ...
        if time.ticks_diff(time.ticks_ms(), context.get("init_start_time", 0)) >= 2000:
            info("状态: {} ,初始化阶段完成, 发布init_complete事件", event, module="FSM")
            return "init_complete"

    elif event == "exit":
        info("状态: {} ,退出初始化状态", event, module="FSM")
...
def networking_state_handler(event, context):
    ...
    if event == "enter":
        ...
        if elapsed > timeout:
            ...
            if network_manager:
                try:
                    network_connected = network_manager.is_connected()
            ...
            if network_connected:
                warning(
                    "网络连接超时({}ms),网络已连接但MQTT可能失败,进入RUNNING状态",
                    elapsed,
                    module="FSM",
                )
                return "running"  # 网络已连接，进入RUNNING状态
            else:
                error(
                    "网络连接完全超时({}ms),网络连接失败,进入ERROR状态",
                    elapsed,
                    module="FSM",
                )
                return "error"  # 网络连接失败，进入ERROR状态
...
def running_state_handler(event, context):
    """运行状态处理函数"""
    if event == "enter":
        info("进入RUNNING状态, 网络连接已完成, 系统正常运行中", module="FSM")
        ...
        # 注意：RUNNING状态的网络维护由_periodic_maintenance统一处理，避免重复调用

    elif event == "wifi_disconnected":
        # WiFi连接断开, 转换到NETWORKING状态重新连接
        warning("WiFi连接断开, 转换到NETWORKING状态", module="FSM")
        return "networking"

    elif event == "mqtt_disconnected":
        # MQTT连接断开, 但WiFi保持连接, 只记录日志不转换状态
        warning("MQTT连接断开, 保持WiFi连接, 尝试重新连接MQTT", module="FSM")
        # 尝试重新连接MQTT, 但不转换状态
        _try_reconnect_mqtt(context)
        return None
...
def recovery_state_handler(event, context):
    ...
    if event == "enter":
        ...
        # 10秒后完成恢复
        elapsed = time.ticks_diff(
            time.ticks_ms(), context.get("recovery_start_time", 0)
        )
...
def _start_network_connection(context):
    ...
    try:
        info("启动网络连接...", module="FSM")
        network_manager.connect()
    except Exception as e:
        error("启动网络连接失败: {}", e, module="FSM")
        context["event_bus"].publish(
            EVENTS["SYSTEM_ERROR"], f"网络连接启动失败: {str(e)}"
        )
...
def _log_system_status(context):
    """记录系统状态"""
    try:
        running_duration = time.ticks_diff(
            time.ticks_ms(), context.get("running_start_time", 0)
        )
...
def _check_system_health(context):
    ...
    try:
        ...
        memory_threshold = (
            context["config"].get("daemon", {}).get("memory_threshold", 80)
        )
        ...
        # 检查网络状态
        network_manager = context.get("network_manager")
        if network_manager and hasattr(network_manager, "is_connected"):
            network_connected = network_manager.is_connected()
            if not network_connected:
                warning("网络连接已断开", module="FSM")
                context["event_bus"].publish(
                    EVENTS["SYSTEM_STATE_CHANGE"],
                    state="warning",
                    warning_msg="网络连接已断开",
...
def _try_reconnect_mqtt(context):
    """尝试重新连接MQTT，不影响WiFi连接"""
    try:
        network_manager = context.get("network_manager")
        if not network_manager:
            return
        ...
        # 检查网络管理器是否有MQTT重连功能
        if hasattr(network_manager, "reconnect_mqtt"):
            info("调用网络管理器的MQTT重连功能", module="FSM")
            network_manager.reconnect_mqtt()
        elif hasattr(network_manager, "loop"):
            info("通过loop()方法让网络管理器处理MQTT重连", module="FSM")
            # 网络管理器会在loop中处理MQTT重连
            pass
        else:
            warning("网络管理器不支持MQTT重连", module="FSM")

    except Exception as e:
        error("MQTT重连尝试失败: {}", e, module="FSM")
