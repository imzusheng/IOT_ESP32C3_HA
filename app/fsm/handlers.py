...
def boot_state_handler(event, context):
    """启动状态处理函数"""
    if event == 'enter':
        info("进入启动状态，系统开始初始化", module="FSM-Boot")
        ...
        if time.ticks_diff(time.ticks_ms(), context.get('boot_start_time', 0)) >= 1000:
            info("启动阶段完成，发布boot_complete事件", module="FSM-Boot")
            return 'boot_complete'
    
    elif event == 'exit':
        info("退出启动状态", module="FSM-Boot")
...
def init_state_handler(event, context):
    """初始化状态处理函数"""
    if event == 'enter':
        info("进入初始化状态，开始模块初始化", module="FSM-Init")
        ...
        if time.ticks_diff(time.ticks_ms(), context.get('init_start_time', 0)) >= 2000:
            info("初始化阶段完成，发布init_complete事件", module="FSM-Init")
            return 'init_complete'
    
    elif event == 'exit':
        info("退出初始化状态", module="FSM-Init")
...
def networking_state_handler(event, context):
    """网络连接状态处理函数"""
    if event == 'enter':
        info("进入NETWORKING状态，开始WiFi连接", module="FSM-Networking")
        ...
        timeout = context['config'].get('wifi', {}).get('connection_timeout', 60000)
        
        if elapsed > timeout:
            warning("WiFi连接超时（{}ms），发布超时事件", elapsed, module="FSM-Networking")
            return 'error'
    
    elif event == 'exit':
        info("退出NETWORKING状态", module="FSM-Networking")
...
def running_state_handler(event, context):
    """运行状态处理函数"""
    if event == 'enter':
        info("进入RUNNING状态，系统正常运行中", module="FSM-Running")
        info("NTP同步已完成，系统时间已同步", module="FSM-Running")
        info("将在1秒后异步启动MQTT连接", module="FSM-Running")
        ...
        info("退出RUNNING状态", module="FSM-Running")
...
def warning_state_handler(event, context):
    """警告状态处理函数"""
    if event == 'enter':
        warning("进入WARNING状态，系统出现警告", module="FSM-Warning")
        ...
        if elapsed >= 30000:  # 30秒
            if not context.get('recovery_triggered', False):
                ...
                info("警告状态超时，尝试恢复", module="FSM-Warning")
                return 'recovery_success'
    
    elif event == 'exit':
        info("退出WARNING状态", module="FSM-Warning")
...
def error_state_handler(event, context):
    """错误状态处理函数"""
    if event == 'enter':
        error("进入ERROR状态，系统出现错误", module="FSM-Error")
        ...
        if elapsed >= 15000:  # 15秒
            if not context.get('recovery_triggered', False):
                context['recovery_triggered'] = True
                info("错误状态超时，尝试恢复", module="FSM-Error")
                return 'recovery_success'
    
    elif event == 'exit':
        info("退出ERROR状态", module="FSM-Error")
        context.pop('recovery_triggered', None)
...
def safe_mode_state_handler(event, context):
    """安全模式状态处理函数"""
    if event == 'enter':
        info("进入安全模式 - 执行紧急清理", module="FSM-SafeMode")
        ...
        if time.ticks_diff(current_time, context.get('last_gc_time', 0)) >= 30000:
            ...
            try:
                gc.collect()
                info("安全模式垃圾回收", module="FSM-SafeMode")
            except Exception as e:
                info("安全模式垃圾回收失败: {}", e, module="FSM-SafeMode")
    
    elif event == 'exit':
        info("退出安全模式", module="FSM-SafeMode")
        
        # 重置错误计数
        context['error_count'] = 0
        info("错误计数已重置", module="FSM-SafeMode")
...
def recovery_state_handler(event, context):
    """恢复状态处理函数"""
    if event == 'enter':
        info("进入RECOVERY状态，开始系统恢复", module="FSM-Recovery")
        ...
        if elapsed >= 10000:  # 10秒
            if not context.get('completion_triggered', False):
                ...
                info("恢复超时，自动完成恢复", module="FSM-Recovery")
                return 'recovery_success'
    
    elif event == 'exit':
        info("退出RECOVERY状态", module="FSM-Recovery")
...
def shutdown_state_handler(event, context):
    """关机状态处理函数"""
    if event == 'enter':
        info("进入SHUTDOWN状态，系统准备关机", module="FSM-Shutdown")
        ...
        info("退出SHUTDOWN状态（这通常不应该发生）", module="FSM-Shutdown")
...
def _perform_initialization(context):
    """执行初始化任务"""
    try:
        info("执行系统初始化任务", module="FSM-Init")
        ...
        error("初始化任务执行失败: {}", e, module="FSM-Init")
...
def _start_wifi_connection(context):
    ...
    if not wifi_manager:
        error("WiFi管理器不可用", module="FSM-Networking")
    ...
    try:
        if hasattr(wifi_manager, 'is_connected') and wifi_manager.is_connected():
            info("进入NETWORKING状态时WiFi已连接，快速切换到RUNNING状态", module="FSM-Networking")
            context['event_bus'].publish(EVENTS.WIFI_STATE_CHANGE, state='connected')
            return
    except Exception as e:
        error("NETWORKING快速路径检查失败: {}", e, module="FSM-Networking")
    
    # 启动WiFi连接过程
    try:
        if hasattr(wifi_manager, 'connect'):
            info("启动WiFi连接过程", module="FSM-Networking")
            wifi_manager.connect()
        elif hasattr(wifi_manager, 'update'):
            wifi_manager.update()
    except Exception as e:
        error("启动WiFi连接失败: {}", e, module="FSM-Networking")
...
def _schedule_mqtt_connection(context):
    ...
    if not mqtt_controller:
        info("MQTT控制器不可用，跳过连接", module="FSM-Running")
        return
    
    try:
        info("开始异步MQTT连接", module="FSM-Running")
        mqtt_controller.connect()
        info("MQTT连接请求已发送，等待异步结果", module="FSM-Running")
    except Exception as e:
        error("MQTT连接失败: {}", e, module="FSM-Running")
...
def _log_system_status(context):
    ...
    try:
        ...
        info("RUNNING - 内存使用: {:.1f}% (空闲: {}KB)", mem_percent, mem_free // 1024, module="FSM-Running")
        
    except Exception as e:
        error("RUNNING - 状态检查失败: {}", e, module="FSM-Running")
...
def _check_system_health(context):
    ...
    try:
        ...
        if mem_percent > memory_threshold:
            warning("高内存使用率: {:.1f}%", mem_percent, module="FSM-Running")
        ...
        try:
            ...
            if temp:
                ...
                if temp > temp_threshold:
                    warning("高温: {}°C", temp, module="FSM-Running")
                    context['event_bus'].publish(EVENTS.SYSTEM_STATE_CHANGE, 
                                               state='warning', 
        ...
        error("健康检查失败: {}", e, module="FSM-Running")

def _emergency_cleanup(context):
    """执行紧急清理"""
    try:
        # 深度垃圾回收
        info("执行深度垃圾回收", module="FSM-SafeMode")
        for _ in range(3):
            gc.collect()
            time.sleep_ms(50)
        ...
        if object_pool and hasattr(object_pool, 'clear_all_pools'):
            info("清理对象池", module="FSM-SafeMode")
        ...
        if mqtt_controller and hasattr(mqtt_controller, 'disconnect'):
            try:
                info("断开MQTT连接", module="FSM-SafeMode")
                mqtt_controller.disconnect()
            except Exception as e:
                info("断开MQTT连接失败（忽略）: {}", e, module="FSM-SafeMode")
        
        # 保存关键状态到缓存
        static_cache = context.get('static_cache')
        if static_cache:
            try:
                info("保存系统状态", module="FSM-SafeMode")
                static_cache.set('safe_mode_entry_time', time.ticks_ms())
                static_cache.save(force=True)
            except Exception as e:
                info("保存系统状态失败（忽略）: {}", e, module="FSM-SafeMode")
    
    except Exception as e:
        # 即使在安全模式下，清理失败也不应该导致系统崩溃
        info("安全模式紧急清理部分失败（继续运行）: {}", e, module="FSM-SafeMode")

def _perform_shutdown_cleanup(context):
    """执行关机清理"""
    try:
        info("执行关机清理工作", module="FSM-Shutdown")
        
        # 断开MQTT连接
        mqtt_controller = context.get('mqtt_controller')
        if mqtt_controller:
            try:
                info("断开MQTT连接", module="FSM-Shutdown")
                mqtt_controller.disconnect()
            except Exception as e:
                info("断开MQTT连接失败（忽略）: {}", e, module="FSM-Shutdown")
        
        # 断开WiFi连接
        wifi_manager = context.get('wifi_manager')
        if wifi_manager:
            try:
                info("断开WiFi连接", module="FSM-Shutdown")
                wifi_manager.disconnect()
            except Exception as e:
                info("断开WiFi连接失败（忽略）: {}", e, module="FSM-Shutdown")
        
        # 关闭LED
        led_controller = context.get('led_controller')
        if led_controller:
            try:
                info("关闭LED", module="FSM-Shutdown")
                led_controller.cleanup()
            except Exception as e:
                info("关闭LED失败（忽略）: {}", e, module="FSM-Shutdown")
        
        # 保存关键数据到缓存
        static_cache = context.get('static_cache')
        if static_cache:
            try:
                info("保存系统状态", module="FSM-Shutdown")
                static_cache.set('shutdown_time', time.ticks_ms())
                static_cache.save(force=True)
            except Exception as e:
                info("保存系统状态失败（忽略）: {}", e, module="FSM-Shutdown")
        
        info("关机清理完成", module="FSM-Shutdown")
        
    except Exception as e:
        info("关机清理过程中出现错误（忽略）: {}", e, module="FSM-Shutdown")