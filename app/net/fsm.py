...
class NetworkFSM:
    ...
    def _on_wifi_state_change(self, state, info=None):
        """处理WiFi状态变化"""
        debug("[FSM-DEBUG] WiFi状态变化: {} -> {}, 当前FSM状态: {}", 
                         'connected' if self.wifi_connected else 'disconnected', 
                         state, STATE_NAMES[self.current_state], module="NET_FSM")
        
        if state == 'connected':
            self.wifi_connected = True
            info("WiFi连接成功", module="NET_FSM")
            debug("[FSM-DEBUG] WiFi连接成功，准备进入MQTT连接阶段", module="NET_FSM")
            # WiFi连接成功，继续MQTT连接
    ...
    def _on_mqtt_callback(self, state, error=None):
        ...
        if state == 'connected':
            ...
            info("MQTT连接成功", module="NET_FSM")
            debug("[FSM-DEBUG] MQTT连接成功，网络连接完成", module="NET_FSM")
            if self.current_state == STATE_CONNECTING:
                self._handle_event('connection_success')
        elif state == 'disconnected':
            self.mqtt_connected = False
            debug("[FSM-DEBUG] MQTT断开，WiFi状态: {}, FSM状态: {}", 
                             self.wifi_connected, STATE_NAMES[self.current_state], module="NET_FSM")
            # 如果WiFi已连接但MQTT断开，只重连MQTT，不重新连接WiFi
            if self.wifi_connected:
                if self.current_state == STATE_CONNECTED:
                    info("MQTT断开但WiFi正常，只重连MQTT", module="NET_FSM")
                    debug("[FSM-DEBUG] 智能重连策略：保持WiFi连接，仅重连MQTT", module="NET_FSM")
                    self._reconnect_mqtt_only()
                elif self.current_state == STATE_CONNECTING:
                    info("MQTT连接失败但WiFi正常，只重连MQTT", module="NET_FSM")
                    debug("[FSM-DEBUG] 连接阶段MQTT失败，启用智能重连", module="NET_FSM")
                    # 在连接过程中MQTT失败，只重连MQTT，不触发整个网络流程失败
                    self._reconnect_mqtt_only()
            elif self.current_state == STATE_CONNECTED:
                # WiFi也断开了，触发连接丢失
                debug("[FSM-DEBUG] WiFi和MQTT都断开，触发连接丢失事件", module="NET_FSM")
                self._handle_event('connection_lost')
    
    def _on_mqtt_message(self, topic, message):
        """MQTT消息回调"""
        info("收到MQTT消息: 主题={}, 消息={}", topic, message, module="NET_FSM")
    ...
    def _on_ntp_state_change(self, state, **kwargs):
        """处理NTP状态变化"""
        if state == 'success':
            debug("NTP同步成功", module="NET_FSM")
        elif state == 'failed':
            debug("NTP同步失败", module="NET_FSM")
    
    def _handle_event(self, event):
        """处理内部事件"""
        info("处理事件: {} (当前状态: {})", event, STATE_NAMES[self.current_state], module="NET_FSM")
        ...
        if next_state is not None and next_state != self.current_state:
            ...
            warning("事件 {} 在状态 {} 中没有定义转换", event, STATE_NAMES[self.current_state], module="NET_FSM")
        else:
            debug("事件 {} 在状态 {} 中不会触发状态转换", event, STATE_NAMES[self.current_state], module="NET_FSM")
    ...
    def _on_enter_connecting(self):
        ...
        debug("[FSM-DEBUG] 进入CONNECTING状态，当前WiFi状态: {}, MQTT状态: {}", 
                         self.wifi_connected, self.mqtt_connected, module="NET_FSM")
        
        # 智能连接策略：如果WiFi已连接，只重连MQTT
        if self.wifi_connected:
            info("WiFi已连接，直接连接MQTT", module="NET_FSM")
            debug("[FSM-DEBUG] 智能连接策略：跳过WiFi连接，直接连接MQTT", module="NET_FSM")
            self._connect_mqtt()
        else:
            info("WiFi未连接，开始完整连接流程", module="NET_FSM")
            debug("[FSM-DEBUG] 完整连接流程：先连接WiFi，再连接MQTT", module="NET_FSM")
            self._connect_wifi()
    
    def _on_enter_connected(self):
        """进入已连接状态"""
        self.retry_count = 0
        info("网络连接完成", module="NET_FSM")
    
    def _on_enter_error(self):
        """进入错误状态"""
        error("网络连接失败", module="NET_FSM")
        
        debug("[FSM-DEBUG] 进入ERROR状态，WiFi: {}, MQTT: {}, 重试次数: {}/{}", 
        ...
        if self.wifi_connected and not self.mqtt_connected:
            # 只有MQTT失败，WiFi正常
            info("WiFi连接正常，仅MQTT连接失败，准备重连MQTT", module="NET_FSM")
            debug("[FSM-DEBUG] MQTT连接失败详情 - 可能原因：服务器未启动、网络不通、认证失败", module="NET_FSM")
            # 设置更短的重连延迟
            delay = min(self.backoff_delay * (2 ** self.retry_count), 30)  # MQTT重连延迟更短
        else:
            # WiFi或其他网络连接失败
            debug("[FSM-DEBUG] WiFi连接失败详情 - 可能原因：信号弱、密码错误、路由器问题", module="NET_FSM")
            delay = min(self.backoff_delay * (2 ** self.retry_count), 60)
        
        info("{}秒后尝试重连", delay, module="NET_FSM")
        debug("[FSM-DEBUG] 重连延迟计算：基础时间={}s, 当前延迟={}s", 
    ...
    def _on_enter_disconnected(self):
        ...
        info("网络已断开", module="NET_FSM")
    
    def _connect_wifi(self):
        """连接WiFi"""
        if self.wifi_connected:
            info("WiFi已连接，跳过连接过程", module="NET_FSM")
            debug("[FSM-DEBUG] WiFi状态检查：已连接，跳过WiFi连接流程", module="NET_FSM")
            return
        
        info("开始连接WiFi", module="NET_FSM")
        debug("[FSM-DEBUG] WiFi连接流程开始，准备扫描网络", module="NET_FSM")
        self.event_bus.publish(EVENTS['WIFI_STATE_CHANGE'], state="connecting")
        
        # 扫描网络
        info("扫描WiFi网络...", module="NET_FSM")
        networks = self.wifi.scan_networks()
        info("找到 {} 个WiFi网络", len(networks) if networks else 0, module="NET_FSM")
        
        if not networks:
            error("未找到可用WiFi网络", module="NET_FSM")
        ...
        info("配置了 {} 个WiFi网络", len(wifi_networks), module="NET_FSM")
        
        connected = False
        for network in networks:
            ssid = network['ssid']
            rssi = network.get('rssi', 'N/A')
            debug("发现WiFi网络: {} (RSSI: {})", ssid, rssi, module="NET_FSM")
            
            for network_config in wifi_networks:
                if network_config.get('ssid') == ssid:
                    password = network_config.get('password')
                    
                    info("尝试连接WiFi: {} (RSSI: {})", ssid, rssi, module="NET_FSM")
                    
                    if self.wifi.connect(ssid, password):
                        info("WiFi连接命令已发送，等待连接建立...", module="NET_FSM")
                        # 等待连接建立
                        ...
                        if connected:
                            info("WiFi连接成功: {}", ssid, module="NET_FSM")
                            debug("[FSM-DEBUG] WiFi连接成功，继续执行MQTT连接", module="NET_FSM")
                            self.event_bus.publish(EVENTS['WIFI_STATE_CHANGE'], state="connected", ssid=ssid)
                            # WiFi连接成功后，继续连接MQTT
                            self._connect_mqtt()
                            return
                        else:
                            warning("WiFi连接超时: {}", ssid, module="NET_FSM")
                    else:
                        error("WiFi连接命令失败: {}", ssid, module="NET_FSM")
        
        # 所有网络都连接失败
        error("所有WiFi网络连接失败", module="NET_FSM")
    ...
    def _connect_mqtt(self):
        ...
        if self.mqtt_connected:
            info("MQTT已连接，跳过连接过程", module="NET_FSM")
            debug("[FSM-DEBUG] MQTT状态检查：已连接，跳过MQTT连接流程", module="NET_FSM")
            return
        
        info("开始连接MQTT", module="NET_FSM")
        debug("[FSM-DEBUG] MQTT连接流程开始，WiFi状态: {}", self.wifi_connected, module="NET_FSM")
        ...
        try:
            info("调用MQTT控制器连接方法...", module="NET_FSM")
            result = self.mqtt.connect()
            info("MQTT连接方法调用完成，结果: {}", result, module="NET_FSM")
            
            # 如果连接方法返回False，记录失败但不立即重连
            if not result:
                error("MQTT连接方法返回失败", module="NET_FSM")
                debug("[FSM-DEBUG] MQTT连接失败，WiFi状态: {}，等待超时处理", self.wifi_connected, module="NET_FSM")
                # 发布MQTT状态为disconnected，让超时机制处理重连
                self.event_bus.publish(EVENTS['MQTT_STATE_CHANGE'], state="disconnected", error="连接失败")
            else:
                info("MQTT连接启动成功，等待连接建立...", module="NET_FSM")
                debug("[FSM-DEBUG] MQTT连接方法调用成功，等待连接回调", module="NET_FSM")
                
        except Exception as e:
            error("MQTT连接启动失败: {}", e, module="NET_FSM")
            debug("[FSM-DEBUG] MQTT连接异常，WiFi状态: {}，等待超时处理", self.wifi_connected, module="NET_FSM")
            # 发布MQTT状态为disconnected，让超时机制处理重连
    ...
    def _sync_ntp(self):
        ...
        try:
            ...
            debug("NTP同步异常: {}", e, module="NET_FSM")
            self.event_bus.publish(EVENTS['NTP_STATE_CHANGE'], state="failed", error=str(e))
    
    def _reconnect_mqtt_only(self):
        """只重连MQTT，不重新连接WiFi"""
        try:
            info("开始MQTT重连（保持WiFi连接）", module="NET_FSM")
            debug("[FSM-DEBUG] MQTT独立重连开始 - WiFi状态: {}, FSM状态: {}", 
                             self.wifi_connected, STATE_NAMES[self.current_state], module="NET_FSM")
            self.event_bus.publish(EVENTS['MQTT_STATE_CHANGE'], state="connecting")
            ...
            if hasattr(self.mqtt, 'reset_failure_count'):
                ...
                debug("[FSM-DEBUG] MQTT失败计数器已重置", module="NET_FSM")
            
            # 启动MQTT连接
            debug("[FSM-DEBUG] 调用MQTT控制器连接方法", module="NET_FSM")
            result = self.mqtt.connect()
            if not result:
                error("MQTT重连启动失败", module="NET_FSM")
                debug("[FSM-DEBUG] MQTT重连失败，但不影响WiFi连接和FSM状态", module="NET_FSM")
                self.event_bus.publish(EVENTS['MQTT_STATE_CHANGE'], state="disconnected", error="连接失败")
            else:
                info("MQTT重连启动成功", module="NET_FSM")
                debug("[FSM-DEBUG] MQTT重连启动成功，等待连接建立", module="NET_FSM")
                # 重置状态开始时间，给MQTT连接更多时间
                self.state_start_time = time.ticks_ms()
        except Exception as e:
            error("MQTT重连异常: {}", e, module="NET_FSM")
            debug("[FSM-DEBUG] MQTT重连异常，但不影响WiFi连接和FSM状态", module="NET_FSM")
            self.event_bus.publish(EVENTS['MQTT_STATE_CHANGE'], state="disconnected", error=str(e))
    ...
    def _disconnect_all(self):
        ...
        try:
            ...
            error("断开连接时发生错误: {}", e, module="NET_FSM")
    ...
    def _check_connection_timeout(self):
        ...
        if self.current_state == STATE_CONNECTING:
            ...
            debug("[FSM-DEBUG] 连接超时检查，已耗时: {}ms，WiFi: {}, MQTT: {}", 
            ...
            if not self.wifi_connected:
                ...
                if elapsed > wifi_timeout:
                    warning("WiFi连接超时（{}ms）", elapsed, module="NET_FSM")
                    debug("[FSM-DEBUG] WiFi连接超时详情 - 超时阈值: {}ms，实际耗时: {}ms", 
                ...
                if elapsed > mqtt_timeout:
                    warning("MQTT连接超时（{}ms），WiFi连接正常", elapsed, module="NET_FSM")
                    debug("[FSM-DEBUG] MQTT连接超时详情 - 超时阈值: {}ms，实际耗时: {}ms，WiFi状态正常", 
                                     mqtt_timeout, elapsed, module="NET_FSM")
                    # WiFi已连接，转换到ERROR状态，让重连机制处理MQTT重连
                    debug("[FSM-DEBUG] MQTT超时，转换到ERROR状态进行智能重连", module="NET_FSM")
                    self._handle_event('connection_failed')
            else:
                # 全部连接成功，不应该还在CONNECTING状态
                if elapsed > self.connection_timeout:
                    warning("连接状态异常，触发重连", module="NET_FSM")
                    debug("[FSM-DEBUG] 连接状态异常详情 - WiFi: {}, MQTT: {}, 但仍在CONNECTING状态", 
                                     self.wifi_connected, self.mqtt_connected, module="NET_FSM")
                    self._handle_event('connection_failed')
    ...
    def _check_retry_timeout(self):
        ...
        if self.current_state == STATE_ERROR:
            if hasattr(self, 'retry_time'):
                ...
                debug("[FSM-DEBUG] 重连超时检查，当前时间差: {}ms，重试次数: {}/{}", 
                                 time_diff, self.retry_count, self.max_retries, module="NET_FSM")
                
                if time_diff >= 0:
                    self.retry_count += 1
                    debug("[FSM-DEBUG] 重连时间到达，开始第{}次重试", self.retry_count, module="NET_FSM")
                    
                    if self.retry_count <= self.max_retries:
                        info("开始第{}次重连尝试", self.retry_count, module="NET_FSM")
                        self._handle_event('retry')
                    else:
                        error("达到最大重试次数", module="NET_FSM")
                        debug("[FSM-DEBUG] 重试次数已达上限 {}/{}，执行重置操作", 
    ...
    def connect(self):
        """启动网络连接"""
        info("NetworkFSM.connect() 被调用", module="NET_FSM")
        info("当前状态: {}", STATE_NAMES[self.current_state], module="NET_FSM")
        
        if self.current_state == STATE_DISCONNECTED:
            info("从DISCONNECTED状态启动连接", module="NET_FSM")
            self._handle_event('connect')
        elif self.current_state == STATE_ERROR:
            info("从ERROR状态重试连接", module="NET_FSM")
            self.retry_count = 0
            self._handle_event('retry')
        else:
            warning("当前状态{}不允许启动连接", STATE_NAMES[self.current_state], module="NET_FSM")
    ...
    def is_connected(self):
        ...
        debug("[FSM-DEBUG] 连接状态检查 - FSM状态: {}, WiFi: {}, MQTT: {}, 最终结果: {}", 
    ...
    def loop(self):
        ...
        try:
            ...
            error("网络状态机循环错误: {}", e, module="NET_FSM")
            debug("[FSM-DEBUG] 状态机循环异常详情 - 当前状态: {}, WiFi: {}, MQTT: {}, 错误: {}", 
